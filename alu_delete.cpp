...
int main( int argc, char* argv[] )
{
  ...
  if ( opt == "preprocess" ) {  
    ...
    delete_search(minLen_alu_del, bam_fh, file_fa_prefix, chrns, fn_tmp1, fn_log1, file_alupos, COVERAGE_HIGH, rg_to_idx);
    delete bam_fh;
    ...
    while (getline(fin, line)) {
      ...
      if ( flagInt > 0) parseline_del(fout, line, pdf_rg, log10RatioUb, flagInt);
    }
    fin.close();
    fout.close();
    ...
    if ( flagInt > 0) parseline_del(cout, line, pdf_rg, log10RatioUb, flagInt, true);
