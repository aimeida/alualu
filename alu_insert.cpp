...
void join_all_loci(string f_in, string f_out, int block_dist, int block_len) {
  ...
  while ( getline(fin, line) ) {
    ss.clear(); ss.str( line );
    ss >> cnt >> pa >> pb;
    // fixme: what if pa <= pb_block ? combine blocks ? otherwise too many !!!
    if ( pa <= pb_block + block_dist and pa <= pa_block + block_len) { // same block
...
void combine_fn(vector<string> & fn_inputs, string fn_output, vector<int> & fn_idx){
  ...
  join_all_loci(fn_output + ".tmp", fn_output, MAX_POS_DIF, MAX_LEN_REGION);
...
bool combine_pn_pos(string chrn, map <int, string> &id_pn_map, string output_prefix, int group_size, string path0) {
  ...
  for (i = 0; i < n_group; i++) {
    fnOut = (n_size <= group_size) ? (fn_prefix + "pos") : (fn_prefix + int_to_string(i) + ".tmp0");
    for ( j = 0; j < group_size; j++) {
      fnOut_fnInput[fnOut].push_back(path0 + id_pn_map[ i*group_size+j ] + ".tmp4st." + chrn);
      fnOut_pnIdx[fnOut].push_back(i*group_size+j);
    }
  }
  fnOut = (n_size <= group_size) ? (fn_prefix + "pos") : (fn_prefix + int_to_string(n_group) + ".tmp0");
  ...
  for (fni = fnOut_fnInput.begin(); fni != fnOut_fnInput.end(); fni ++) 
    ...
    //cout << 0 << " " << fnOut_fnInput.size() << " " << fni->first << " " << (fni->second).size() << endl;	
  ...
  while ( true ) {
      ...
      fnOut_fnInput.clear();
      for (i = 0; i < n_group; i++) { 
	fnOut = (n_size <= group_size) ? (fn_prefix + "pos") : (fn_prefix + int_to_string(i) + ".tmp"+ int_to_string(fn_idx+1) );
	for ( j = 0; j < group_size; j++) fnOut_fnInput[fnOut].push_back( fn_prefix + int_to_string(i*group_size+j) + ".tmp"+ int_to_string(fn_idx) );
      }

      fnOut = (n_size <= group_size) ? (fn_prefix + "pos") : (fn_prefix + int_to_string(n_group) + ".tmp"+ int_to_string(fn_idx+1) );
...
int main( int argc, char* argv[] )
{
  ...
  if (opt == 1) {     // takes about 2.5 hrs 
    ...
    int debug_pn = ID_pn.size() ;
    string output_prefix = path1 + "batch0.";
    map<int, string> id_pn_map;
    map<int, string>::iterator ii = ID_pn.begin();
    for (int i = 0; i < debug_pn; i++) id_pn_map[i] = ID_pn[i];
    for (vector<string>::iterator ci = chrns.begin(); ci != chrns.end(); ci++) 
